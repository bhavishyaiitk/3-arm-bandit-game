...
class BanditEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action == 0:
            reward = np.random.randint(27, 38)  # High reward for Q
        elif action == 1:
            reward = np.random.randint(15, 35)  # Medium reward for W
        elif action == 2:
            reward = np.random.randint(33, 49)  # Low reward for E
...
with open('player_data.csv', mode='a', newline='') as file:
    ...
    while running:
        ...
        # text_number = font.render(str(env.random_number), True, BLACK)
        ...
        elapssed_time = pygame.time.get_ticks() - start_time if start_time is not None else 0

        # Display timer on the screen
        text_timer = font.render("Timer: " + str(int(elapssed_time / 1000)), True, BLACK)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if action is not None:
                    ...
                    writer.writerow([choice_str, reward, reaction_time])  # Write reward and reaction time to CSV
